...
class APIService {
  ...
  async fetchSportsList() {
    ...
    try {
      ...
      // Handle the actual API response format: {"status":"success","data":[{sport_id, sport_name}]}
      if (response.status === 'success' && Array.isArray(response.data)) {
        ...
        console.warn('Unexpected sports response format:', response);
        sportsData = [];
      }
    ...
    try {
      ...
      if (response.status === 'success' && Array.isArray(response.data)) {
        tournamentsData = response.data
          ...
          .map(sport => ({
            // Map to match your component expectations
            sports_id: sport.sport_id, // Your components expect sports_id
            sport_id: sport.sport_id,
            ...
            tournaments: sport.tournaments
              ...
              .map(tournament => ({
                ...
                matches: Array.isArray(tournament.matches) ? tournament.matches.map(match => ({
                  ...match,
                  // Map start_date to start_time for MatchCard compatibility
  ...
  filterTournamentsBySport(tournaments, sportId) {
    ...
    const filtered = tournaments.filter(sport => {
      // Compare numeric sport IDs - use sports_id which is what we set in the mapping
      return sport.sports_id === targetSportId || sport.sport_id === targetSportId;
    });
...
export default new APIService(true); // Set to true for demo API, false for production